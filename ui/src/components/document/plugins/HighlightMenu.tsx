...
function HighlightMenu(props) {
  ...
  useEffect(() => {
    const hyperlink = props.view.state.selection.$head.marks().find((mark) => mark.type.name == "hyperlink")
    if(hyperlink) {
  ...
  function toggleItalic() {
    toggleMark(schema.marks["italic"])(props.view.state, props.view.dispatch, props.view)
  }
  function toggleUnderline() {
    toggleMark(schema.marks["underline"])(props.view.state, props.view.dispatch, props.view)
  }
  function toggleStrike() {
    toggleMark(schema.marks["strike"])(props.view.state, props.view.dispatch, props.view)
  }
  function toggleCode() {
    toggleMark(schema.marks["code"])(props.view.state, props.view.dispatch, props.view)
  }

  function toggleLink() {
    toggleMark(schema.marks["hyperlink"])(props.view.state, props.view.dispatch, props.view)
  }
  ...
  function implementLinkChange() {
    const res = extendMark(props.view.state, props.view.state.selection.from, props.view.state.selection.to, schema.marks["hyperlink"]);
    if(res == null) return
    const remove = props.view.state.tr.removeMark(res.from, res.to, schema.marks["hyperlink"])
    props.view.dispatch(remove)
    const add = props.view.state.tr.addMark(res.from, res.to, schema.marks["hyperlink"].create({ href: link }))
    props.view.dispatch(add)
    console.log(res)
  }

  function hasMark(mark: string): boolean {
    const sel = props.view.state.selection;
    return props.view.state.doc.rangeHasMark(sel.from, sel.to, schema.marks[mark]);
  }
  ...
  return (
    ...
    >
      {
        hasMark("hyperlink") ? (
          <input type="text" value={link} onChange={handleLinkChange} className="px-3 py-2" onBlur={implementLinkChange}/>
        ) : ("")
      }
      <div className="flex">
        <li onClick={toggleBold} style={hasMark("strong") ? { "backgroundColor": "var(--type-glass-color)" } : {} }>
          {/* Bold */}
          <svg
        ...
        <li onClick={toggleItalic} style={hasMark("italic") ? { "backgroundColor": "var(--type-glass-color)" } : {} }>
          {/* Italic */}
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 384 512"
        ...
        <li onClick={toggleUnderline} style={hasMark("underline") ? { "backgroundColor": "var(--type-glass-color)" } : {} }>
          {/* Underline */}
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 448 512"
        ...
        <li onClick={toggleStrike} style={hasMark("strike") ? { "backgroundColor": "var(--type-glass-color)" } : {} }>
          {/* Strike */}
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 512 512"
        ...
        <li onClick={toggleCode} style={hasMark("code") ? { "backgroundColor": "var(--type-glass-color)" } : {} }>
          {/* Code */}
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 576 512"
        ...
        <li onClick={toggleLink} style={hasMark("hyperlink") ? { "backgroundColor": "var(--type-glass-color)" } : {} }>
          {/* Link */}
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 640 512"
