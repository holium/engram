<template>
    <div class="flex flex-col gap-2" @drop="handleDrop" @dragover="handleDragOver">
        ...
        <SystemItem :parent="'.'" :item="item.id" :type="'folder'" :key="item.id" v-for="item in items.filter((item) => item.type == 'folder')" />
        <SystemItem :parent="'.'" :item="item.id" :type="'document'" :key="item.id" v-for="item in items.filter((item) => item.type == 'document')" />
    </div>
</template>
